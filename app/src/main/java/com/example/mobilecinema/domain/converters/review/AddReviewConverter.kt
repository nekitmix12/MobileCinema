...
class AddReviewConverter {
    fun convert(
        getFilmResult: Result<AddReviewUseCase.Response>
    ): UiState<Unit> {
        return when (getFilmResult) {
            is Result.Error -> UiState.Error(getFilmResult.exception)
            is Result.Success -> UiState.Success(getFilmResult.data.response)
        }
    }
}